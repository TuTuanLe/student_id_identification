...
class Giohang extends CI_Controller {
    ...
    public function index(){
        $this->data['title']='ifitness - Giỏ hàng của bạn';
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $this->data['title']='ifitness - Thông tin đơn hàng';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('sessionKhachHang')){
            ...
            $config['smtp_user']    = 'webfiness.ktxa.uit@gmail.com';
            $config['smtp_pass']    = 'rongthieng123';
            ...
            $this->email->from('webfiness.ktxa.uit@gmail.com', 'fitness');
            $list = array($val['email']);
            $this->email->to($list);
            $this->email->subject('FITNESS');
